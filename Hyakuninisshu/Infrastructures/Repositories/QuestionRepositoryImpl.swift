...
extension CDQuestion {
  fileprivate func toModel() -> Question {
    let correctNo = KarutaNo(UInt8(correct_karuta_no))
    guard let cdQuestionChoices: [CDQuestionChoice] = questionChoices?.array as? [CDQuestionChoice]
    else {
    ...
    if start_date != nil {
      if selected_karuta_no != 0 {
        ...
        let result = QuestionResult(
          selectedKarutaNo: KarutaNo(UInt8(selected_karuta_no)), answerSec: answer_time,
          judgement: judgement)
  ...
  fileprivate func persistFromModel(question: Question) {
...
class QuestionRepositoryImpl: QuestionRepository {
  ...
  func findCollection() -> Future<QuestionCollection, DomainError> {
    let publisher = Future<QuestionCollection, DomainError> { promise in
      ...
      let asyncFetch = NSAsynchronousFetchRequest<CDQuestion>(fetchRequest: fetchRequest) {
        result in
        ...
        do {
          ...
          guard let cdQuestion = try context.fetch(fetchRequest).first else {
            let domainError = DomainError(
              reason: "Not found question. id=\(question.id.value)", kind: .repository)
            promise(.failure(domainError))
